...
### 7.1 `idp-agent init` — day 0

Creates the IaC repository and **materialises the governance model**:

```
iac-repo/
├── catalog/{databases,apis,caches}/
│   └── .witness.yml                 ← witness file (§ 4.4)
├── dependencies/{access,network}/
│   └── .witness.yml
├── .github/workflows/validate.yml   ← refuses what the catalogue would accept
├── CODEOWNERS                       ← who is entitled to review
├── schemas/                         ← Zod schemas exported as JSON Schema
└── README.md                        ← the doctrine, written down
```

Plus the application repository's `catalog-info.yml`, generated by inspecting the code.
Everything is deterministic except that last file, which goes through the same
`propose()`.

A repository initialised by `idp-agent` is one where the security model is **true**,
not merely documented: branch protection stops the requester from merging their own
request.

### 7.2 `idp-agent "<intent>"` — day N
...
Direct graph query, tabular output. No plan, no writes.
