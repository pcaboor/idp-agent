...
Record each scenario's exchanges once and replay them by default: `IDP_RECORDING=record
pnpm test` will write `tests/recordings/*.json` with a key, plain `pnpm test` will replay
...
Recordings are a second artefact to maintain: every intended behaviour change needs someone
